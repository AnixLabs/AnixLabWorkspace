  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/users/create" className="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600">
          + Create User
        </Link>
      </div>
          ...
          <Link key={user.id} href={`/users/${user.id}`}>
            <div className="border rounded-xl p-4 hover:shadow-md transition cursor-pointer bg-white dark:bg-neutral-900">
              <div className="flex items-center gap-3 mb-2">
                {user.image ? (
                  <Image src={user.image} alt={user.name} className="w-10 h-10 rounded-full object-cover" width={40} height={40} unoptimized />
                ) : (
              ...
              <p className="text-xs text-gray-300 mt-1">{new Date(user.createdAt).toLocaleDateString()}</p>
